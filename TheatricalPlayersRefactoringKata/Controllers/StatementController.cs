    ...
    public class StatementController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTextBillingStatement([FromBody] Invoice invoice)
        {
            ...
            try
            {
                ...
                string txtBillingStatement = statement.PrintText(invoice, plays);

                string dirPath = Path.Combine(Environment.CurrentDirectory, "statements");
                if (!Directory.Exists(dirPath)) Directory.CreateDirectory(dirPath);
                await System.IO.File.WriteAllTextAsync(Path.Combine(dirPath, $"{DateTime.Now:ddMMyyHHmmss}_{invoice.Customer}_order.txt"), txtBillingStatement);

                return Ok(txtBillingStatement);
            }
