...
public class StatementPrinter
{
    ...
    public StatementData GenerateStatementData(Invoice invoice, Dictionary<string, Play> plays)
    {
        ...
        foreach (var perf in invoice.Performances)
        {
            ...
            statementData.Items.Add(new ItemData
            {
                ...
                EarnedCredits = Math.Max(perf.Audience - 30, 0) + ((play.Type == "comedy") ? (int)Math.Floor((decimal)perf.Audience / 5) : 0),
                Seats = perf.Audience
            });
            totalAmount += thisAmount;  
        }
    ...
    {
        var statementData = GenerateStatementData(invoice, plays);
